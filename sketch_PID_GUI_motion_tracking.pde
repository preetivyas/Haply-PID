 ...
 * @brief      PID example with random position of a target
 ...
 edited by Preeti Vyas on 26th Feb 2021
 **********************************************************************************************************************
 * @attention
 *
 *
...
Knob pknob, dknob, iknob;
Slider s, loopt, rad, thet;
int tagC, tagS, tagH;
...
void setup() {
  ...
  cp5.addTextlabel("Theta")
    .setText("Circle Theta Diff")
    .setPosition(975, 435)
    .setColorValue(color(0, 0, 0))
    .setFont(createFont("Verdana", 17))
    ;  
    
  thet = cp5.addSlider("theta_diff")
    .setPosition(980, 465)
    .setSize(200, 30)
...
public void ResetDevice(int theValue) {
  //reset the end effector position
  widgetOne.device_set_parameters();
}

//toggle button functions

public void Circle(boolean theFlag) {
  if (theFlag==true)
    tagC = 1;
  else
    tagC = 0;
...
//public void Square(boolean theFlag) {
//  if (theFlag==true)
//    tagS = 1;
//  else
//    tagS = 0;
...
//public void Heart(boolean theFlag) {
//  if (theFlag==true)
//    tagH = 1;
//  else
//    tagH = 0;
...
void keyPressed() {
  if (key == 'q') {
    ...
    looptime += 100;
    loopt.setValue(looptime);
  } else if (key == 'f') {
    looptime -= 100;
    loopt.setValue(looptime);
  } else if (key == 't') {
    smoothing += 0.01;
    s.setValue(smoothing);
  } else if (key == 'g') {
    smoothing -= 0.01;
    s.setValue(smoothing);
  } else if (key == ' ') {
...
public void SimulationThread() {
  while (1==1) {
    ...
    iter+= 1;
    println("P:"+ P + " I:" + I + " D:" + D + " looptime:" + looptime +" smoothing:"+ smoothing+ " errorx:" + cumerrorx + " errory:" + cumerrory);
    ...
    if (haplyBoard.data_available()) {
      ...
      // can play with these numbers to avoid instability 
